...
def _parallel_evolve(n_programs, parents, X, Y, seeds, params, sample_weight=None):
    """Private function used to build a batch of programs within a job."""
    n_days, n_samples, n_features = X.shape
    # Unpack parameters
    ...
    def _tournament():
        ...
        fitness = [parents[p].fitness(X, Y) for p in contenders]
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        X : array-like, shape = [n_days, n_samples, n_features]
            Training vectors, where n_days is the number of days,
