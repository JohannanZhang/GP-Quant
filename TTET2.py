...
# 基于固定年份year的变量X、收益Y，以及当年的基准收益，计算个体筛选的适应度
def parallel_evolve(n_programs, parents, X, Y, seeds, year: int):
    ...
    for i in range(n_programs):
        ...
        Program = _Program(ts_function_set=ts_function_set,
                            random_state=random_state,
                            arities=arities,
...
with open('C:/Data/stock_data/factor/gplearn/fuctiontree_save/49_FuncTreelist.pkl', 'rb') as file:
    loaded_lists = pickle.load(file)
parents = loaded_lists
...
for gen in range(generations):
    ...
    with open(f'C:/Data/stock_data/factor/gplearn/fuctiontree_save/{gen+50}_FuncTreelist.pkl', 'wb') as file:
    ...
    for j in population[gen]:
        try:
            ...
            print(f"第{gen + 50 + 1}代,索引为{j}的公式树出现了问题")
    ...
    print(f"第{gen + 51}代的公式树列表为", Programs)
    print(f"第{gen + 51}代最优个体的位置为：{fittest_index}  第{gen + 51}代最优个体的的适应度：{max(fit_list)}  ")
    print(f"第{gen + 51}代的年化超额收益为：{excess_ret_2017}")
...
# fitness = [program.raw_fitness(X, Y) for program in population]
# length = [program.length_ for program in population]
# print(population[-1])
# fit_list = []
# for j in population[-1]:
#     try:
#         fit = j.raw_fitness(X, Y)
#         fit_list.append(fit)
#     except Exception as e:
#         print(f"索引为{j}的公式树出现了问题")
# print(f"第{generations}代最优个体的的适应度：", max(fit_list))




# def calculate_fitness(start, end, parents):
#     fit_list = []
#     for i in range(start, end):
#         fit_list.append(parents[i].raw_fitness(X, Y))
#     return fit_list


# for i in range(10):
#     n_jobs, n_programs, starts = _partition_estimators(population_size, n_jobs=n_jobs)
#     # 使用joblib的Parallel和delayed方法来并行计算每个元素的函数值
#     result = Parallel(n_jobs=12, verbose=5)(delayed(calculate_fitness)(starts[i], starts[i+1], parents) for i in range(n_jobs))
#
...
# abs_Result = [abs(i) for i in Result]
# print(max(abs_Result))
# index = abs_Result.index(max(abs_Result))
# print(index)
# print(Result[index])
...
# from custom_program import make_ts_function
# function = make_ts_function(ts_function_set[7],
#                             [4, 5, 6, 7, 8, 9, 10], random_state)
#
# parent1.program = [function, 8]
# pro1 = parent1.program
# # 查看公式树的元素名称
# s = []
# for i in pro1:
#     if isinstance(i, _Function):
#         i = i.name
#         s.append(i)
#     else:
#         s.append(i)
