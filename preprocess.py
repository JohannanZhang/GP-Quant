...
# 得到分别由开盘、盘中、收盘数据组成的dataframe
# open_a = a[a['time_range'] == '开盘']
# intra_a = a[a['time_range'] == '盘中']
# close_a = a[a['time_range'] == '尾盘']
...
#         group['price_range'] = ['high' if price >= median_price else 'low' for price in group['close']]
#         return group
#     # 分组并应用划分价格区域的函数
...
# 截面分位点标准化
# def section_quantile_standlize(df: pd.DataFrame, column_name: str):
#     quantiles = df[column_name].quantile([0.25, 0.5, 0.75])  # 计算分位点
#     df[column_name] = (df[column_name] - quantiles[0.5]) / (quantiles[0.75] - quantiles[0.25])
#     return df
